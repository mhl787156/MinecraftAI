...
public class RangeSensor {
	...
	public Line2D getShape() {
		Line2D.Double line = new Line2D.Double(agent.getPosition(),
				getEndpoint());
		return line;
	}
	 ...
	 */
	public double getX() {
		return (SENSOR_RANGE * Math.cos(getSensorAngle()))
	 ...
	 */
	public double getY() {
		return (SENSOR_RANGE * Math.sin(getSensorAngle()))
	...
	// we can't use the standard Java2D intersection here because
	// it assumes infinitely long lines, which means we get intersections with
	// the obstacles in both directions (i.e. both 0 degrees and 180 degrees).
	...
	public void paint(Graphics2D g, double pixelRatio) {
		...
		// blend so we don't completely mask the path.
		g.setComposite(AlphaComposite
				.getInstance(AlphaComposite.SRC_OVER, 0.5f));
		g.setColor(Color.RED);

		// draw the closest intersection with obstacles.
		Point2D inters = getNearestObstacleIntersection();

		// draw the rangefinder line.
		Point2D center = new Point2D.Double(agent.getShape().getCenterX(),
				agent.getShape().getCenterY());
		Point2D endpoint = getEndpoint();
		g.draw(ShapeHelper.getLine(new Line2D.Double(center, inters),
				pixelRatio));
	...
	public double getDistanceToNearestObstacle() {
		...
		// if there is no nearest obstacle, then the nearest obstacle is effectively 
		// the furthest possible distance away.
